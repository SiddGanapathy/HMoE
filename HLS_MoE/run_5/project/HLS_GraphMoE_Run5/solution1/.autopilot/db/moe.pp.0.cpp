...
__attribute__((sdx_kernel("graph_moe", 0))) void graph_moe(
    ...
    const float w1[4][5][32][128],
    const float b1[4][5][32],

    const float w2[4][5][16][32],
    const float b2[4][5][16],

    const float w3[4][5][8][16],
    const float b3[4][5][8],

    const float w4[4][5][1][8],
    const float b4[4][5][1],
...
#pragma HLS INLINE off
 ...
 float l1[32];
    ...
    VITIS_LOOP_48_1: for (int i = 0; i < 32; i++)
    {
        float sum = b1[i];

        VITIS_LOOP_52_2: for (int j = 0; j < 128; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w1[i][j] * input[j];
    ...
    VITIS_LOOP_62_3: for (int i = 0; i < 16; i++)
    {
        float sum = b2[i];

        VITIS_LOOP_66_4: for (int j = 0; j < 32; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w2[i][j] * l1[j];
        }
    ...
    VITIS_LOOP_76_5: for (int i = 0; i < 8; i++)
    {
        float sum = b3[i];

        VITIS_LOOP_80_6: for (int j = 0; j < 16; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w3[i][j] * l2[j];
        }
    ...
    VITIS_LOOP_92_7: for (int j = 0; j < 8; j++)
    {
#pragma HLS PIPELINE II=1
 result += w4[0][j] * l3[j];
    }
...
#pragma HLS INLINE off
 ...
 float l1[32];
    ...
    VITIS_LOOP_140_1: for (int i = 0; i < 32; i++)
    {
        float sum = b1[i];

        VITIS_LOOP_144_2: for (int j = 0; j < 128; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w1[i][j] * input[j];
        }
    ...
    VITIS_LOOP_154_3: for (int i = 0; i < 16; i++)
    {
        float sum = b2[i];

        VITIS_LOOP_158_4: for (int j = 0; j < 32; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w2[i][j] * l1[j];
        }
    ...
    VITIS_LOOP_168_5: for (int i = 0; i < 8; i++)
    {
        float sum = b3[i];

        VITIS_LOOP_172_6: for (int j = 0; j < 16; j++)
        {
#pragma HLS PIPELINE II=1
 sum += w3[i][j] * l2[j];
        }
    ...
    VITIS_LOOP_184_7: for (int j = 0; j < 8; j++)
    {
#pragma HLS PIPELINE II=1
 result += w4[0][j] * l3[j];
    }
...
{
#line 18 "/home/simics/HMoE/HLS_HMoE/run_5/scripts/run_hls.tcl"
...
#pragma HLS INTERFACE mode=s_axilite port=return
 ...
 float logits[4];
    ...
    VITIS_LOOP_248_1: for (int e = 0; e < 4; e++)
    {
        float sum = gate_bias[e];

        VITIS_LOOP_252_2: for (int i = 0; i < 128; i++)
        {
#pragma HLS PIPELINE II=1
 sum += gate_weight[e][i] * input[i];
    ...
    VITIS_LOOP_268_3: for (int e = 1; e < 4; e++)
    {
    ...
    VITIS_LOOP_277_4: for (int e = 0; e < 4; e++)
    {
        exp_logits[e] = expf(logits[e] - max_logit);
        exp_sum += exp_logits[e];
    }

    VITIS_LOOP_283_5: for (int e = 0; e < 4; e++)
    {
        gates[e] = exp_logits[e] / exp_sum;
    }
# 306 "src/moe.cpp"
    float expert_output[4][5];





    VITIS_LOOP_312_6: for (int obj = 0; obj < 5; obj++)
    {
    ...
    VITIS_LOOP_354_7: for (int obj = 0; obj < 5; obj++)
    {
    ...
    VITIS_LOOP_396_8: for (int obj = 0; obj < 5; obj++)
    {
        float sum = 0.0f;
