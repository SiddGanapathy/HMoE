# 1 "/home/simics/HMoE/HLS_HMoE/pseudo_block/run_5/src/pseudo_moe.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/simics/HMoE/HLS_HMoE/pseudo_block/run_5/src/pseudo_moe.cpp" 2
...
# 2 "/home/simics/HMoE/HLS_HMoE/pseudo_block/run_5/src/pseudo_moe.cpp" 2
...
#pragma HLS ARRAY_PARTITION variable=w_local cyclic factor=8 dim=1
 ...
 VITIS_LOOP_25_1: for (int k = 0; k < D * D; k++) {
#pragma HLS PIPELINE II=1
 w_local[k] = w[k];
    }

    VITIS_LOOP_30_2: for (int o = 0; o < D; o++) {
        float partial[LANES];
#pragma HLS ARRAY_PARTITION variable=partial complete


 VITIS_LOOP_35_3: for (int lane = 0; lane < LANES; lane++) {
#pragma HLS UNROLL
 partial[lane] = 0.0f;
        }


        VITIS_LOOP_41_4: for (int g = 0; g < GROUPS; g++) {
#pragma HLS PIPELINE II=1

 VITIS_LOOP_44_5: for (int lane = 0; lane < LANES; lane++) {
#pragma HLS UNROLL
 const int i = g * LANES + lane;
                partial[lane] += x[i] * w_local[o * D + i];
            }
        }


        float acc = b[o];
        VITIS_LOOP_53_6: for (int lane = 0; lane < LANES; lane++) {
#pragma HLS UNROLL
 acc += partial[lane];
        }
...
#line 14 "/home/simics/HMoE/HLS_HMoE/pseudo_block/run_5/scripts/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=pseudo_moe
# 75 "/home/simics/HMoE/HLS_HMoE/pseudo_block/run_5/src/pseudo_moe.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 float logits[E];
    ...
    VITIS_LOOP_98_1: for (int n = 0; n < N; n++) {
#pragma HLS PIPELINE off


 float max_logit = -1.0e30f;

        VITIS_LOOP_104_2: for (int e = 0; e < E; e++) {
            float acc = b_gate[e];

            VITIS_LOOP_107_3: for (int d = 0; d < D; d++) {
        ...
        VITIS_LOOP_121_4: for (int e = 0; e < E; e++) {
            exp_values[e] = expf(logits[e] - max_logit);
            exp_sum += exp_values[e];
        }

        VITIS_LOOP_126_5: for (int e = 0; e < E; e++) {
            gates_out[n * E + e] = exp_values[e] / exp_sum;
        }


        VITIS_LOOP_131_6: for (int d = 0; d < D; d++) {
            output[n * D + d] = 0.0f;
        }


        compute_expert(&input[n * D], w0, b0, expert_result);

        VITIS_LOOP_138_7: for (int o = 0; o < D; o++) {
            output[n * D + o] +=
                gates_out[n * E + 0] * expert_result[o];
        }


        compute_expert(&input[n * D], w1, b1, expert_result);

        VITIS_LOOP_146_8: for (int o = 0; o < D; o++) {
            output[n * D + o] +=
                gates_out[n * E + 1] * expert_result[o];
        }


        compute_expert(&input[n * D], w2, b2, expert_result);

        VITIS_LOOP_154_9: for (int o = 0; o < D; o++) {
            output[n * D + o] +=
                gates_out[n * E + 2] * expert_result[o];
        }


        compute_expert(&input[n * D], w3, b3, expert_result);

        VITIS_LOOP_162_10: for (int o = 0; o < D; o++) {
            output[n * D + o] +=
