# update_run9_resource.py

from pathlib import Path

SRC = Path("src/moe.cpp")

text = SRC.read_text()

# ------------------------------------------------------------
# Locate the generic MLP section.
# Everything from this point onward is regenerated.
# ------------------------------------------------------------

marker = "static void generic_mlp_head"

pos = text.find(marker)

if pos < 0:
    raise RuntimeError(
        "Could not find 'static void generic_mlp_head' in src/moe.cpp"
    )

# Preserve:
#   - headers
#   - all 164 weight arrays
prefix = text[:pos]

# ------------------------------------------------------------
# New resource-efficient implementation
# ------------------------------------------------------------
...
#pragma HLS ARRAY_PARTITION variable=output complete
    ...
    for (int e = 0; e < NUM_EXPERTS; e++) {
        gates[e] = hls::expf(logits[e] - max_logit);
        exp_sum += gates[e];
    }
...
SRC.write_text(prefix + suffix)

print("Updated:", SRC)
print("Preserved generated weight arrays.")
print("Installed one reusable sequential expert engine.")
print("Removed concurrent DATAFLOW engines.")
