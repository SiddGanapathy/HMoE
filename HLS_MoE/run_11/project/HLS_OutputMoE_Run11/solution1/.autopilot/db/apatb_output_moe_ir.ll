...
define void @apatb_output_moe_ir(float* noalias nocapture nonnull readonly "fpga.decayed.dim.hint"="128" %input, float* noalias nocapture nonnull "fpga.decayed.dim.hint"="5" %output, float* noalias nocapture nonnull "fpga.decayed.dim.hint"="4" %gates) local_unnamed_addr #0 {
entry:
  ...
  call fastcc void @copy_in([128 x float]* nonnull %0, [128 x float]* nonnull align 512 %input_copy, [5 x float]* nonnull %1, [5 x float]* nonnull align 512 %output_copy, [4 x float]* nonnull %2, [4 x float]* nonnull align 512 %gates_copy)
  call void @apatb_output_moe_hw([128 x float]* %input_copy, [5 x float]* %output_copy, [4 x float]* %gates_copy)
  call void @copy_back([128 x float]* %0, [128 x float]* %input_copy, [5 x float]* %1, [5 x float]* %output_copy, [4 x float]* %2, [4 x float]* %gates_copy)
  ret void
}

; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @copy_in([128 x float]* noalias readonly, [128 x float]* noalias align 512, [5 x float]* noalias readonly, [5 x float]* noalias align 512, [4 x float]* noalias readonly, [4 x float]* noalias align 512) unnamed_addr #1 {
entry:
...
; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @copy_out([128 x float]* noalias, [128 x float]* noalias readonly align 512, [5 x float]* noalias, [5 x float]* noalias readonly align 512, [4 x float]* noalias, [4 x float]* noalias readonly align 512) unnamed_addr #4 {
entry:
...
declare void @apatb_output_moe_hw([128 x float]*, [5 x float]*, [4 x float]*)

; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @copy_back([128 x float]* noalias, [128 x float]* noalias readonly align 512, [5 x float]* noalias, [5 x float]* noalias readonly align 512, [4 x float]* noalias, [4 x float]* noalias readonly align 512) unnamed_addr #4 {
entry:
  call fastcc void @onebyonecpy_hls.p0a5f32([5 x float]* %2, [5 x float]* align 512 %3)
  call fastcc void @onebyonecpy_hls.p0a4f32([4 x float]* %4, [4 x float]* align 512 %5)
  ret void
...
define void @output_moe_hw_stub_wrapper([128 x float]*, [5 x float]*, [4 x float]*) #5 {
entry:
  call void @copy_out([128 x float]* null, [128 x float]* %0, [5 x float]* null, [5 x float]* %1, [4 x float]* null, [4 x float]* %2)
  %3 = bitcast [128 x float]* %0 to float*
  %4 = bitcast [5 x float]* %1 to float*
  %5 = bitcast [4 x float]* %2 to float*
  call void @output_moe_hw_stub(float* %3, float* %4, float* %5)
  call void @copy_in([128 x float]* null, [128 x float]* %0, [5 x float]* null, [5 x float]* %1, [4 x float]* null, [4 x float]* %2)
  ret void
}
